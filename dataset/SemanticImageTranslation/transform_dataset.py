...
# Ensure this script's folder (SemanticImageTranslation) is first on sys.path so that
# `src/` and `utils/` here are preferred over any top-level `src/` in the repo.
THIS_DIR = Path(__file__).resolve().parent
if str(THIS_DIR) not in sys.path:
    sys.path.insert(0, str(THIS_DIR))

from src.diffedit_editer import DiffEditEditer
...
def main(sample_df, args):
    ...
    for i, l in sample_df.iterrows():
        ...
        out_imgs = out_imgs if isinstance(out_imgs, dict) else dict(images=out_imgs) # convert PIL Image
