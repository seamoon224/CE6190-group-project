...
def evaluate(args):    
    ...
    img_paths = [
        x for x in args.folder.iterdir()
        if x.name.lower().endswith(('.png', '.jpg', '.jpeg'))
    ]
    print(img_paths)
    present_ids = [int(x.name[:-5]) for x in img_paths] # 因为是jpeg，所以-4改成-5
    ...
    if args.metrics['lpips'] or args.metrics['l1']:       
        input_paths = [Path(GLOBAL['IMAGENET_ROOT']) / df.loc[i].path.split('/')[-1] for i in present_ids]
